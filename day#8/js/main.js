...
let countiesInfoList = [];
for (let values of Object.values(countries)) {
  countiesInfoList.push([
    `${values.name}'s capital is ${values.capital}. The population of ${
      values.name
    } is ${values.population} people, who speak ${values.languages.join(
      ", "
    )} ${"languag" + (values.languages.length === 1 ? "e" : "es")}.`,
  ]);
}
